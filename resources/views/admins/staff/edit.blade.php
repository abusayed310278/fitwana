...
<div class="content-wrapper">
    <div class="page-header d-flex justify-content-between align-items-center">
        <div>
            <h3 class="page-title">Edit User - {{ $staff->name }}</h3>
                    ...
                    <form action="{{ route('staff.update', $staff) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('display_name') is-invalid @enderror"
                                       id="display_name" name="display_name" value="{{ old('display_name', $staff->display_name) }}">
                        ...
                        <div class="row">
                                ...
                                <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required>
                                    @foreach($roles as $role)
                                        <option value="{{ $role->name }}"
                                                @if(old('role', $staff->roles->first()?->name) == $role->name) selected @endif>
                                            {{ ucfirst($role->name) }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                    <option value="active" @if(old('status', $staff->email_verified_at ? 'active' : 'suspended') == 'active') selected @endif>
                                        Active
                                    </option>
                                    <option value="suspended" @if(old('status', $staff->email_verified_at ? 'active' : 'suspended') == 'suspended') selected @endif>
                                        Suspended
                                    </option>
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('whatsapp') is-invalid @enderror"
                                   id="whatsapp" name="whatsapp" value="{{ old('whatsapp', $staff->whatsapp) }}"
                            ...
                            <div class="text-center mb-3">
                                <div class="profile-photo-container">
                                    ...
                                    <div class="remove-photo-btn" title="Remove photo"
                                         style="{{ $staff->profile_photo_url ? 'display: flex;' : 'display: none;'}}"
                                         onclick="removePhoto();">
                                        <i class="ti-close"></i>
                                    </div>
                                </div>
                                <input type="file" id="photoUpload" name="profile_photo_url" class="d-none" accept="image/*">
                                <small class="form-text text-muted">Click on the image to upload a new photo. JPG, PNG max 2MB.</small>
                            </div>
                        </div>
                ...
                <div class="card-body">
                    ...
                    <div class="d-grid gap-2">
                        <button type="button" class="btn btn-warning" onclick="resetPassword({{ $staff->id }})">
                            <i class="ti-key"></i> Reset Password
                        </button>

                        <button type="button" class="btn btn-info" onclick="toggleStatus({{ $staff->id }})">
                            <i class="ti-power-off"></i>
                            {{ $staff->email_verified_at ? 'Suspend User' : 'Activate User' }}
                        </button>

                        <a href="{{ route('staff.show', $staff) }}" class="btn btn-outline-primary">
                            <i class="ti-eye"></i> View Details
                        </a>
                    </div>

                    <hr>
                    <h6 class="fw-bold">User Statistics</h6>
                    <div class="row text-center">
                        <div class="col-6">
                            <div class="border rounded p-2">
                                <h5 class="mb-1 text-primary">{{ $staff->orders->count() ?? 0 }}</h5>
                                <small class="text-muted">Orders</small>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="border rounded p-2">
                                <h5 class="mb-1 text-success">{{ $staff->subscriptions->count() ?? 0 }}</h5>
